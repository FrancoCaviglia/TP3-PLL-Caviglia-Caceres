...
def crear_figura(cord_x, cord_y, leyenda_x, leyenda_y, encabezado):
    ...
    plt.title(r'\texttt{'+leyenda_y+'}', fontsize=11)
    plt.xlabel(r'\texttt{'+leyenda_x+'}', fontsize=11)
    plt.ylabel(r'\texttt{'+encabezado+'}', fontsize=11)
    #   plt.margins(0,0.05)
    plt.grid()
...
def actualizar_figura(cord_x, cord_y, leyenda_x, leyenda_y, encabezado, i):
    
    global canvas_lista, plots, canvas_widget, figuras
    ...
    #canvas_widget[i].delete("all")
    #del canvas_widget[i]
    
    plots[i] = crear_figura(cord_x, cord_y, leyenda_x, leyenda_y, encabezado)  # Fabrica la figura correspondiente
...
ent_muestras = tk.Entry(master=frame_2_2_1_1, width=7)
...
ent_freq     = tk.Entry(master=frame_2_2_1_1, width=7)
...
ent_amort    = tk.Entry(master=frame_2_2_1_1, width=7)
...
ent_freq_m   = tk.Entry(master=frame_2_2_1_1, width=7)
...
ent_freq_p   = tk.Entry(master=frame_2_2_1_1, width=7)
...
imgArea = canvas_figura.create_image(346, 84, anchor = tk.CENTER, image = img)
...
figura_1 = tk.Frame(master=frame_2_2_2, width=20, height=300, bg="white", relief=tk.RAISED, borderwidth=1)
figura_2 = tk.Frame(master=frame_2_2_2, width=20, height=300, bg="white", relief=tk.RAISED, borderwidth=1)
figura_3 = tk.Frame(master=frame_2_2_2, width=20, height=300, bg="white", relief=tk.RAISED, borderwidth=1)
figura_1.grid(row=0, column=0, padx = 3, pady = 4, sticky='nsew')
figura_2.grid(row=0, column=1, padx = 3, pady = 4, sticky='nsew')
figura_3.grid(row=0, column=2, padx = 3, pady = 4, sticky='nsew')        
...
def botones_grafica(frame, j):
    ...
    button = tk.Button(text='Tiempo', width=9, height=1, bg="#C6C6C6", fg="black", master=frame)
    button.grid(row=0, column=6, sticky = 'nsew')

    button = tk.Button(text='Frecuencia', width=9, height=1, bg="#C6C6C6", fg="black", master=frame)
    button.grid(row=0, column=7, sticky = 'nsew')
        
    button = tk.Button(text='Lissajous', width=9, height=1, bg="#C6C6C6", fg="black", master=frame)
    button.grid(row=0, column=8, sticky = 'nsew')
...
def ejercicio_boton(i): # Combina el ejercicio con el cambio de imagen
    ...
    global img, canvas_figura, datos, ejercicio_enunciado, ejercicio_respuesta
    
    datos = ejercicios[i](ejercicio_enunciado, ejercicio_respuesta)
...
for i in range(45):    # Agarra cada función con ejercicios y crea nuevas funciones con los argumentos
    ejercicios_arg.append(partial(ejercicio_boton, i))#, datos)) 
