...
""" En este archivo se encuentran las 45 funciones con nombre ejercicio_X que se corresponden con cada
    ...
    las gráfics, luego se pasan esos datos a la lista "datos" y finalmente se retornan en orden dicha lista,
...
def ejercicio_16():
    ...
    entrada  = unir_lista(fuente(f, Ts, 0, N//2), fuente(f, Ts, pi/2, N//2))
    _, error_1, _, salida_1 = PLL(entrada, 'rc', sqrt(2)/2, wn, f0, K, Ts)
    _, error_2, _, salida_2 = PLL(entrada, 'rc', sqrt(2)/2, wn, f0, K, Ts)
    _, error_3, _, salida_3 = PLL(entrada, 'rc', sqrt(2)/2, wn, f0, K, Ts)
    
    datos[0] = lissajous(entrada_1, salida_1)
    datos[1] = error(entrada_1, salida_1)
    datos[2] = lissajous(entrada_2, salida_2)
    datos[3] = error(entrada_2, salida_2)
    datos[4] = lissajous(entrada_3, salida_3)
    datos[5] = error(entrada_3, salida_3)
...
def ejercicio_17():
    ...
    datos[3] = lissajous(entrada, salida)
    datos[4] = error(entrada, salida)
    datos[5] = [[],[]]
