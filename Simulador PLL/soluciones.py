...
def ejercicio_1():
    ...
    return datos, ['Tiempo (s)', 'Amplitud (V)'], [1000, 10000, 1000, 'π/2', '', '', '', '', '', '', '','']
...
def ejercicio_7():
    ...
    salida_2 = filtro(entrada_2, zi, 'rc', 0.25, 0.25, 0, Ts)[0]
...
def ejercicio_8():
    ...
    datos[1] = conversor(filtro_traducido(entrada, zi, 'rc',   2, sqrt(2)/2, 0.0001)[0])
    datos[2] = conversor(filtro_traducido(entrada, zi, 'rc', 100, sqrt(2)/2, 0.0001)[0])
    datos[3] = conversor(filtro_traducido(entrada, zi, 'rc',   2,         5, 0.0001)[0])
    
    return datos, ['Tiempo (s)', 'Amplitud (V)'], [1000, 100000, 1000, '0, π/4', '√ 2/2, √ 2/2, 5', '2, 100, 2', '', '', '', 'rc', '', '']
...
def ejercicio_9():
    ...
    Ts = 0.00001
    zi = []
    entrada = detector(fuente(f, Ts, 0, N), fuente(f, Ts, pi/4, N))
    
    datos[0] = conversor(entrada)
    datos[1] = conversor(filtro_traducido(entrada, zi, 'rc',   2, sqrt(2)/2, Ts)[0])
    datos[2] = conversor(filtro_traducido(entrada, zi, 'rc', 100, sqrt(2)/2, Ts)[0])
    datos[3] = conversor(filtro_traducido(entrada, zi, 'rc',   2,         5, Ts)[0])
        
    return datos, ['Tiempo (s)', 'Amplitud (V)'], [N, 1/Ts, f, '0, π/4', '√2/2, √2/2, 5', '2, 100, 2', '', '', '', 'rc', '', '']
...
def ejercicio_10():
    ...
    datos[0] = conversor(entrada)
    datos[1] = conversor(filtro_traducido(entrada, None, 'lead-lag activo',   2, sqrt(2)/2, Ts)[0])
    datos[2] = conversor(filtro_traducido(entrada, None, 'lead-lag activo', 100, sqrt(2)/2, Ts)[0])
    datos[3] = conversor(filtro_traducido(entrada, None, 'lead-lag activo',   2,         5, Ts)[0])
...
def ejercicio_11():
    ...
    return datos, ['Tiempo (s)', 'Fase VCO'], [N, 1/Ts, f0, '', '', '', '', K, '', '', '', '']
...
def ejercicio_15():
    ...
    return datos, ['Tiempo (s)', 'Amplitud (V)'], [N, fs, f0, '', '', '', '', K, f0, 'rc', '', '']
...
def ejercicio_17():
    ...
    entrada  = unir_lista(fuente(f, Ts, 0, N//2), fuente(f, Ts, pi/2, N//2))
    _, _, salida_filtro, salida_vco = PLL(entrada, 'rc', xi, wn, f0, K, Ts)
...
def ejercicio_18():
    ...
    entrada  = unir_lista(fuente(f, Ts, 0, N//2), fuente(f, Ts, pi/2, N//2))
    _, _, salida_filtro, salida_vco = PLL(entrada, 'lead-lag activo', xi, wn, f0, K, Ts)
...
def ejercicio_41():
    ...
    emisora = []
    for i in range(N):
        emisora.append([moduladora[i][0], filtrado[i]*moduladora[i][1]])
...
def ejercicio_42():
    ...
    datos[3] = [conversor(onda_5)[1], onda_6]
