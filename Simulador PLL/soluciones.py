...
def gen_cod(Nd, Ts, Tb):
    ...
    for i in range(0, Nd):
        salida.append([ (i*step+int(step/2))*Ts, np.round(np.random.rand()) ] )        
...
def ejercicio_39():
    ...
    datos[0] = conversor(np.convolve(gen_cod(Nd, Ts, Tb), respuesta_filtro(N, a, Tb, Fs), 'same'))
    datos[1] = [[],[]]
    datos[2] = [[],[]]
    datos[3] = [[],[]]
...
def ejercicio_41():
    ...
    filtrado   = np.convolve(entrada, respuesta_filtro(N, a, Tb, Fs), 'same')
...
def ejercicio_42():
    ...
    onda_2     = np.convolve(onda_1, respuesta_filtro(N, a, Tb, Fs), 'same')
    moduladora = fuente(1.05*f0,Ts,0,N)     
    onda_3     = multiplicador(onda_2, moduladora)

    xi    = np.sqrt(2)/2   # Parámetros del PLL
    omega = 120
    K     = 1
    tipo  = 'rc'
    ...
    onda_6 = np.convolve(onda_5, 0.01*respuesta_filtro(1,200), 'same')    
