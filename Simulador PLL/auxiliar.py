...
def emisor(Nd, f0, dt, Tb = 2):
    """ Simula el comportamiento del transmisor en cada uno de sus pasos. 
    ...
    -------   
    x  : list
        Señal de N = Nd*Tb/ds elementos de la forma [x,y] de salida representada con una lista con N = Nd*Tb/Ts elementos de la forma [x,y].
    xv : list
        Señal de N = Nd*Tb/ds elementos de la forma [x,y] que representa la moduladora.

    """        
    fs = 200        # Frecuencia de muestreo interna
    Ts = 1/fs       # Período de muestreo interno
    N  = Nd*Tb*fs   # Cantidad de muestras
    a  = 0.5        
    Fs = 1
    
    codigo = gen_cod(Nd, Ts, Tb)
    moduladora = convolve(conversor(codigo)[1], respuesta_filtro(N, a, Tb, Fs)[1], 'same')
    xm = [] # Fabrica la señal moduladora en el formato usual
    for i in range(N):
        xm.append([codigo[i][0],moduladora[i]])
    
    portadora = fuente(f0,Ts,0,N) 
    x = [] # Fabrica la señal moduladora en el formato usual
    for i in range(N):
        x.append([portadora[i][0], moduladora[i]*portadora[i][1]])
